...
def sync(client: TypeSafeClient) -> None:
    ...
    assert_type(
        client.system_one("x", {"q": {"type": "noul", "instructions": "?", "weight": 2}}, retry=RetryPolicy(max_retries=1)), SystemOneResponse
    )
...
async def asynchronous(client: AsyncTypeSafeClient) -> None:
    ...
    assert_type(await client.system_one("x", {"q": {"type": "noul", "instructions": "?", "weight": 2}}, retry=RetryPolicy()), SystemOneResponse)
